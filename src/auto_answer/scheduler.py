...
from .capture import FrameSource, crop_regions
from .config import AppConfig
from .debug import DebugRecorder
from .errors import ADBError, OCRError, SolverError, UnsafeOCRResult
from .models import CroppedFrame, OCRBundle, OCRResult, Question, SchedulerState, SolveDecision
from .ocr import PaddleOCRReader
from .ollama import OllamaClient
from .rules import RuleEngine
from .state import PageStateDetector, ReadyQuestionPage, crop_for_state, extract_question_number
from .text import assemble_question
        ...
        try:
            while max_questions is None or completed < max_questions:
                if ready_page is None:
                    ...
                    excluded = blocked_after_tap or deferred_after_failure
                    ...
                    if ready_page is None:
                        ...
                        # A content failure is retried after one full wait cycle. A successfully
                        # tapped number remains blocked to prevent a duplicate tap.
                ...
                self._logger.info(
                    ...
                    last_read.page.question_number,
                    question.text,
                    question.options,
                )
                ...
                try:
                    ...
                    self._logger.error(
                        "solver failed for question %d; returning to page wait: %s",
                        last_read.page.question_number,
                        exc,
                    )
                    deferred_after_failure = last_read.page.question_number
                    ready_page = None
                    continue
                ...
                old_number = last_read.page.question_number
